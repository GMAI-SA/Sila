...
/// The TrustNet button. **Component 1 of 13.**
///
...
///
/// ```swift
/// TNButton("Send Code", variant: .primary, isLoading: vm.isSending) {
///     await vm.sendCode()
/// }
/// ```
public struct TNButton: View {

    /// Visual weight of a ``TNButton``.
    public enum Variant {
    ...
    /// Vertical footprint of a ``TNButton``.
    public enum Size {
        case regular, compact

        var height: CGFloat { self == .regular ? 52 : 40 }
        var font: Font { self == .regular ? TNFont.bodyEmphasis : TNFont.caption }
    }
    ...
    private var background: some View {
        switch variant {
        case .primary:
            TNColor.brandGradient
        case .secondary:
            TNColor.surface2.opacity(colorScheme == .dark ? 0.6 : 1)
        case .destructive:
            TNColor.danger
        case .ghost:
            Color.clear
    ...
    private var foreground: Color {
        ...
        case .secondary: return TNColor.textPrimary
        case .destructive: return .white
        case .ghost: return TNColor.primary
        }
    }

    private var strokeColor: Color {
        variant == .secondary ? TNColor.primary.opacity(0.45) : .clear
    }
}

#Preview("TNButton") {
    VStack(spacing: TNSpacing.md) {
        TNButton("Create Account", variant: .primary) {}
        TNButton("Sign In", variant: .secondary) {}
        TNButton("Delete Account", variant: .destructive) {}
        TNButton("Resend code", variant: .ghost, size: .compact) {}
        TNButton("Sending…", variant: .primary, isLoading: true) {}
        TNButton("Disabled", variant: .primary, isEnabled: false) {}
        TNButton("Face ID", variant: .secondary, icon: "faceid") {}
    }
    .padding()
