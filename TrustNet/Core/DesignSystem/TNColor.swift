...
/// The TrustNet brand palette.
///
/// TrustNet is **dark-mode first** — the dark values are the canonical brand
/// colours and the light values are derived adaptations. Every colour resolves
/// through ``TNColor/adaptive(dark:light:)`` so a component only has to read
/// `TNColor.background` and the correct value falls out of the environment's
/// `UITraitCollection`.
///
/// ```swift
/// Text("Hello").foregroundStyle(TNColor.textPrimary)
/// ```
public enum TNColor {
...
#Preview("Palette") {
    ScrollView {
        VStack(alignment: .leading, spacing: 8) {
            swatch("primary", TNColor.primary)
            swatch("secondary", TNColor.secondary)
            swatch("danger", TNColor.danger)
            swatch("warning", TNColor.warning)
            swatch("background", TNColor.background)
            swatch("surface1", TNColor.surface1)
            swatch("surface2", TNColor.surface2)
            swatch("textPrimary", TNColor.textPrimary)
            swatch("textSecondary", TNColor.textSecondary)
            swatch("textMuted", TNColor.textMuted)
        }
        .padding()
    }
...
private func swatch(_ name: String, _ color: Color) -> some View {
    HStack {
        ...
        Text(name).foregroundStyle(TNColor.textPrimary)
    }
}
