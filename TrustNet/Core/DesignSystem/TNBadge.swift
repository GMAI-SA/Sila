...
/// never interactive — use ``TNChip`` for anything tappable.
///
/// ```swift
/// TNBadge("Verified", style: .verified, icon: "checkmark.seal.fill")
/// ```
public struct TNBadge: View {

    /// Semantic colouring of a ``TNBadge``.
    public enum Style: Sendable {
        ...
        var tint: Color {
            switch self {
            case .verified: return TNColor.primary
            case .premium: return Color(tnHex: 0xFFC24B)
            case .danger: return TNColor.danger
            case .warning: return TNColor.warning
            case .neutral: return TNColor.textSecondary
            }
        }
    }
    ...
    public var body: some View {
        HStack(spacing: 4) {
            ...
            Text(text.uppercased()).font(TNFont.micro).tracking(0.6)
        }
...
#Preview("TNBadge") {
    VStack(spacing: TNSpacing.md) {
        TNBadge("Verified", style: .verified, icon: "checkmark.seal.fill")
        TNBadge("Premium", style: .premium, icon: "star.fill")
        TNBadge("Rejected", style: .danger, icon: "xmark.octagon.fill")
        TNBadge("Action required", style: .warning, icon: "exclamationmark.triangle.fill")
        TNBadge("Unstarted", style: .neutral)
    }
    .padding()
