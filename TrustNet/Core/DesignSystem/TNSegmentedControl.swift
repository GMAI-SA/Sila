...
///
/// ```swift
/// TNSegmentedControl(items: FeedTab.allCases, selection: $tab) { $0.title }
/// ```
public struct TNSegmentedControl<Item: Hashable & Identifiable>: View {
    ...
    private func segment(_ item: Item) -> some View {
        ...
        return Button {
            ...
            VStack(spacing: TNSpacing.sm) {
                Text(title(item))
                    .font(isSelected ? TNFont.bodyEmphasis : TNFont.body)
                    .foregroundStyle(isSelected ? TNColor.textPrimary : TNColor.textSecondary)
                    .lineLimit(1)
