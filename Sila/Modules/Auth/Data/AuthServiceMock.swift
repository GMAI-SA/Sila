...
public actor AuthServiceMock: AuthServiceProtocol {
    ...
    public init(
        ...
        self.biometricAccount = hasBiometricCredential ? "saved@sila.app" : nil
        if hasBiometricCredential {
            self.storedPair = Self.makePair(email: "saved@sila.app", scenario: scenario)
        }
    }
    ...
    public func register(email: String, password: String) async throws -> RegistrationResult {
        ...
        try await delay()
        try failIfOffline()
        if email.lowercased() == "taken@sila.app" {
