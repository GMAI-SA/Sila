...
///
/// On Sila a country code is not decoration: it comes from the Nafath
...
/// a phone prefix, a locale — is exactly the thing Sila exists not to do.
///
/// ```swift
/// SLCountryBadge(countryCode: post.author.countryCode)
/// ```
public struct SLCountryBadge: View {
    ...
    public var body: some View {
        if let code = CountryCode.normalised(countryCode), let flag = CountryCode.flag(code) {
            ...
            .padding(.horizontal, size == .regular ? SLSpacing.sm : 0)
            .padding(.vertical, size == .regular ? 2 : 0)
            .background {
                if size == .regular {
                    Capsule().fill(SLColor.secondary.opacity(0.12))
                }
            }
...
#Preview("SLCountryBadge") {
    VStack(alignment: .leading, spacing: SLSpacing.lg) {
        HStack(spacing: SLSpacing.md) {
            SLCountryBadge(countryCode: "SA")
            SLCountryBadge(countryCode: "jp")
            SLCountryBadge(countryCode: "BR")
        }
        SLCountryBadge(countryCode: "SA", size: .regular)
        HStack {
            Text("Unverified author:").font(SLFont.caption).foregroundStyle(SLColor.textSecondary)
            SLCountryBadge(countryCode: nil)
            Text("(renders nothing)").font(SLFont.micro).foregroundStyle(SLColor.textMuted)
        }
    }
    .padding()
